...
instance (Eq a, Eq b) => Eq (List a b) where
  Nil == Nil = True
  Cons a1 b1 ls1 == Cons a2 b2 ls2 = a1 == a2 && b1 == b2 && ls1 == ls2
  _ == _ = False

instance Functor (List a) where
  fmap f Nil           = Nil
  fmap f (Cons a b ls) = Cons a (f b) (f `fmap` ls)

instance Foldable (List a) where
  foldr _ n Nil = n
  foldr f n (Cons a b ls) = f b $ foldr f n ls

instance Traversable (List a) where
  traverse f (Cons a b ls) = Cons a <$> f b <*> traverse f ls
  traverse f Nil           = pure Nil

unpack :: List a b -> ([a], [b])
unpack (Cons a b ls) = (a:as, b:bs)
  where
    (as, bs) = unpack ls
unpack Nil           = ([], [])
...
pMul :: Parser Exp
pMul = foldl1 Mul <$> sepBy1 pNotLvl (char' '*')
...
binOp opName e1 e2 f = do
  ...
  v2 <- evalExp e2
  case (f, v1, v2) of
    (Left f, I n1, I n2)  -> pure (I (f n1 n2))
    (Right f, B b1, B b2) -> pure (B (f b1 b2))
...
evalSt s = case s of
  ...
  While e p -> do
    ...
    case v of
      B b ->
        if b
          then newScope (evalProg p) >> evalSt (While e p)
          else pure ()
